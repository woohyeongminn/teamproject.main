        ...
        <div class="col-12">
          <div class="card my-4">
            ...
            <div class="bg-gradient-success shadow-success border-radius-lg pt-4 pb-3" style="background-color: #49a3f1;box-shadow: 0 4px 20px 0 rgb(0 0 0 / 14%), 0 7px 10px -5px rgb(73 163 241 / 40%) !important;">
                <h6 class="text-white text-capitalize ps-3">${adminAskBoard.askTitle}</h6>
              </div>
            </div>
              ...
              <div class="table-responsive p-0">
                <table class="table align-items-center mb-0">
                  ...
                  <tr id="first">
                    ...
                    <th scope="col" id="title"><input id='f-title' type='text' name='title' value="${adminAskBoard.askTitle}" readonly></th>
                    <th scope="col" id="date">${adminNotice.adminNotiRegisteredDate}</th>
                  </tr>
                  
                  <tr id="blockbox">
                    <th scope="row" id="second"><label for='f-content'>내용</label></th>
                    <th scope="col" id="content"><input id='f-content' type='text' name='content' value="${adminAskBoard.askContent}" readonly></th>
                    <th></th>
                  </tr>
                  
                  <tr id="blockbox">
                    <th scope="row" id="second"><label for='f-writer'>작성자</label></th>
                    <th scope="col" id="writer"><input id='f-writer' type='text' name='writer' value="${adminAskBoard.askMemberWriter.perNickname}" readonly></th>
                    <th></th>
                  </tr>
                  
                  <c:if test="${empty adminAskBoard.reply}">
                  <tr id="reply">
                    <th scope="col"><label for='f-reply'>답변</label></th>
                      <th scope="col" id="reply" style="font-size: 14px;">등록된 답변이 없습니다.</th>
                      <th scope="col" id="date"></th>
                  </tr>
                  </c:if>
                  
                  <c:if test="${not empty adminAskBoard.reply}">
                  <tr>
                    <th scope="col"><label for='f-reply'>답변 제목</label></th>
                      <th scope="col" id="reply">
                      <input id='f-reply' type='text' name='reply' value="${adminAskBoard.reply.replyTitle}" readonly></th>
                    <th scope="col" id="date">${adminAskBoard.reply.replyRegisteredDate}</th>
                  </tr>
                  <tr>
                    <th scope="col"><label for='f-replycontent'>답변 내용</label></th>
                      <th scope="col" id="replycontent">
                      <input id='f-replycontent' type='text' name='replycontent' value="${adminAskBoard.reply.replyContent}" readonly></th>
                    <th scope="col"></th>
                  </tr>
                  </c:if>
                ...
                <c:when test="${empty adminAskBoard.reply}">
                  <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                     <button class = "btn btn-outline-dark" type="submit" value="답변" formaction="reply">
                      <a href='${contextPath}/app/askboard/replyaddform?askNo=${adminAskBoard.askNo}'>답변등록</a>
                     </button>
                     <button class = "btn btn-outline-dark" type="submit" value="삭제" formaction="delete">
                     <a href='delete?askNo=${adminAskBoard.askNo}'>삭제</a>
                     </button>   
                  </div>
                </c:when>
                <c:otherwise>
